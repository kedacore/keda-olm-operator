...
func (r *SecretReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if kedaController.Status.SecretDataSum == "" {
		...
		r.Log.Info("Secret containing Certificates was created for the first time -> do nothing")
	} else {
		...
		if kedaController.Status.SecretDataSum == newCheckSum {
			...
			r.Log.Info("Secret containing Certificates was changed -> let's restart KEDA Metrics Server")
